...
void BulletManager::Update(float deltaTime)
{
	for(int i=0;i<m_bullet.size();i++){
		//Ãä¬É§P©w
		if( m_bullet[i].m_pos.y > 400 || m_bullet[i].m_pos.y < -400 ||
			m_bullet[i].m_pos.x > 300 || m_bullet[i].m_pos.x < -300 )
	...
	for(int i=0;i<m_bullet.size();i++){
		for(int j=0;j<enemyM->m_enemyPlane.size();j++){
			...
			if( m_bullet[i].isCollide( enemyM->m_enemyPlane[j] ) ){
				...
				enemyM->m_enemyPlane[j]->m_isHurting = true;
				m_bullet.erase(m_bullet.begin()+i);
				break;
			}
		}
