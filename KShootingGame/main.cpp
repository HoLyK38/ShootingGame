...
void Display(void) 
{
	...
	mainPlaneQ->SetPoint(mainPlane->m_pos.x,mainPlane->m_pos.y);
	mainPlaneQ->Render();
	enemyM->Update(deltaTime);
	enemyM->hitMainPlane(mainPlaneQ);
	mainPlane->m_hp = mainPlaneQ->m_hp;
	//---------Script-----------------------------------
	Script();
	HpDraw(mainPlane->m_hp,mainPlane->m_hpMax);
	if (boss->m_isDead) isGameover = true;
	if(playTitle==false)StageTile->Render();
	}
	else{
		Sleep(1500);
		if(boss->m_isDead)
...
void Keyboard(unsigned char key, int x, int y) 
{
	...
	switch (key)
	{
		...
		case 'x':
			mainPlaneQ->m_hp += 10;
			if(mainPlaneQ->m_hp>100)
				mainPlaneQ->m_hp = 100;
			mainPlaneQ->m_hpMax = 100;
			break;
		case 'v':
			mainPlaneQ->m_hp = 9999;
			mainPlaneQ->m_hpMax = 9999;
			mainPlane->m_hpMax = mainPlaneQ->m_hpMax;
			break;
		case 'c':
			boss->m_hp -= 250;
			break;
...
void Init()
{
	...
	number[0] = new Ball(numberTex,clock_x,clock_y,60,88);
	number[1] = new Ball(numberTex,clock_x+60,clock_y,40,88);
	number[2] = new Ball(numberTex,clock_x+100,clock_y,60,88);
	number[3] = new Ball(numberTex,clock_x+160,clock_y,60,88);
	for(int i=0;i<4;i++)
