...
void MoveTo(float x,float y)
{
	if( (target->m_pos.x>=x && target->m_pos.x<x+5) && 
		(target->m_pos.y>=y && target->m_pos.y<y+5) )
	{
		target->m_state = NONE;
	}else{
		float temp_x,temp_y;
		temp_x = x - target->m_pos.x;
		temp_y = y - target->m_pos.y;
...
void AtkTimer(int value)
{
	...
	if( value==0 && isPlay==false ){
		...
		target->SetPoint(target->m_pos.x,target->m_pos.y,atkTexCoord[value][1]-atkTexCoord[value][0],target->m_height,0);
...
void BossHpDraw()
{
	if(	target->m_hp>0	){
		...
		float a = 300 * p;
		float b = 300 * (1-p)/2;
		bossHP->SetPoint( 480-b , bossHP->m_pos.y  , a , bossHP->m_height,0);
		bossHP->Render();
	}
	bossHPt->Render();
}
void TitleTimer(int value)
{
	...
	if( value==0 && isPlay==false ){
		...
		StageTile->SetPoint(StageTile->m_pos.x,StageTile->m_pos.y + 1);
		glutTimerFunc(200,TitleTimer,value+1);
		clockReset = true;
		gameTime = 0;
	}else{
...
void InitBoss()
{
	...
	StageTile = new Ball(StageTileTex,0,0,184*2,26*2);
